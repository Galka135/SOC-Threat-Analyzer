...
מעל מנוע ה-Verdict הדטרמיניסטי יושבת שכבת **אנליסט AI** אופציונלית עם שני
ספקים המנוסים לפי הסדר: **Gemini קודם** (שכבה חינמית), ו-**Claude כגיבוי** אם
Gemini נכשל או הגיע למכסה. המנוע המספרי נשאר מקור האמת הרפרודוקטיבי; ה-AI מוסיף
את מה שנוסחה אינה יכולה:
...
השכבה אופציונלית לחלוטין: אפשר להגדיר `GEMINI_API_KEY` (חינמי,
[aistudio.google.com/apikey](https://aistudio.google.com/apikey)), `ANTHROPIC_API_KEY`,
או שניהם. בלי אף מפתח — הפאנל פשוט מוסתר והאפליקציה עובדת כרגיל. מודלי ברירת המחדל:
`gemini-2.5-flash` ו-`claude-haiku-4-5-20251001` (ניתן לשנות עם `GEMINI_MODEL` / `AI_MODEL`).
הפאנל מציין איזה ספק ענה בפועל.
...
GEMINI_API_KEY = "..."       # אופציונלי — שכבת AI, מנוסה קודם (חינמי)
ANTHROPIC_API_KEY = "..."    # אופציונלי — שכבת AI, גיבוי ל-Gemini
AI_MODEL = "..."             # אופציונלי — עקיפת מודל Claude (וגם GEMINI_MODEL ל-Gemini)
```
