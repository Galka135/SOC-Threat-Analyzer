...
מעל מנוע ה-Verdict הדטרמיניסטי יושבת שכבת **אנליסט AI** אופציונלית —
Gemini (חינמי) כספק ראשי ו-Claude כגיבוי.
...
השכבה אופציונלית לחלוטין: ללא `GEMINI_API_KEY` ו-`ANTHROPIC_API_KEY` הפאנל פשוט
מוסתר והאפליקציה עובדת כרגיל. מספיק מפתח אחד — Gemini (חינמי) נוסה ראשון, ואם
נכשל הבקשה עוברת ל-Claude. מודלי ברירת המחדל: `gemini-2.5-flash` (שינוי עם
`GEMINI_MODEL`) ו-`claude-haiku-4-5-20251001` (שינוי עם `AI_MODEL`).
...
GEMINI_API_KEY = "..."       # אופציונלי — אנליסט AI, ספק ראשי (חינמי)
ANTHROPIC_API_KEY = "..."    # אופציונלי — אנליסט AI, ספק גיבוי
GEMINI_MODEL = "..."         # אופציונלי — עקיפת מודל ברירת המחדל של Gemini
AI_MODEL = "..."             # אופציונלי — עקיפת מודל ברירת המחדל של Claude
```
